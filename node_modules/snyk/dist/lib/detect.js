...
async function isIacProject(root, options) {
    if (options.file) {
    ...
    switch (projectType) {
        ...
        case constants_1.IacProjectType.TERRAFORM: {
            const { isValidTerraformFile, reason, } = await iac_parser_1.makeValidateTerraformRequest(fileContent);
            if (!isValidTerraformFile) {
                throw invalid_iac_file_1.IllegalTerraformFileError([root], reason);
            }
            break;
        }
        default:
